...
votes = pd.read_csv("voting_data/Canada1972.txt", sep=" ", header=0)
...
with open('voting_data/Canada1972.txt') as file:
    ...
    for line in file:
        ...
        if winner == int(results[0]):
            ...
            colour.append(No_affiliation)
            win.append("No Affiliation")
        else:
            continue
...
sorted_parliament_seats = parliament_charts.create_parliament_seating_plan_1972(PROGRESSIVE_CONSERVATIVE_SEATS, LIBERAL_SEATS, NEW_DEMOCRATIC_SEATS, SOCIAL_CREDIT_SEATS, INDEPENDENT_SEATS, NO_AFFILIATION_SEATS)

parliament_chart = parliament_charts.generateParliamentChart(total_seats, sorted_parliament_seats)

with open("pages/main/parliament_charts/parl_chart1972.html", "w") as file:
...
foliumMap = dataframe3.explore(
    ...
    cmap=['#847e7e', '#EE3224', '#F58220', '#000000', '#0F2D52', '#005F00'],
    style_kwds=dict(color="black"), #use black outline
)

foliumMap.save("./pages/elections/election1972.html")


### Generate election page in main folder
generateHtmlMapFiles.generateElectionMapFile(1972)
