...
Bloc_populaire = '#00A7EC'
Labor_Progressive = '#C00767'
...
votes = pd.read_csv("voting_data/Canada1945.txt", sep=" ", header=0)
...
with open('voting_data/Canada1945.txt') as file:
    ...
    for line in file:
        ...
        if winner == int(results[0]):
            ...
            colour.append(Bloc_populaire)
            win.append("Bloc populaire")
        elif winner == int(results[6]):
            colour.append(Labor_Progressive)
            win.append("Labor-Progressive")
...
sorted_parliament_seats = parliament_charts.create_parliament_seating_plan_1945(PROGRESSIVE_CONSERVATIVE_SEATS, LIBERAL_SEATS, CC_FEDERATION_SEATS, INDEPENDENT_SEATS, SOCIAL_CREDIT_SEATS, BLOC_POPULAIRE, LABOR_PROGRESSIVE, LIBERAL_PROGRESSIVE_SEATS)

parliament_chart = parliament_charts.generateParliamentChart(total_seats, sorted_parliament_seats)

with open("pages/main/parliament_charts/parl_chart1945.html", "w") as file:
...
foliumMap = dataframe3.explore(
    ...
    cmap=['#00A7EC', '#FF9900', '#847e7e', '#C00767', '#EE3224', '#00DCB0', '#0F2D52', '#005F00'],
    style_kwds=dict(color="black"), #use black outline
)

foliumMap.save("./pages/elections/election1945.html")

### Generate election page in main folder
generateHtmlMapFiles.generateElectionMapFile(1945)
